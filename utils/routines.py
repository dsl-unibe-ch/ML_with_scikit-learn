...
def load_multidimensional_data_pca2(eps=0.06, ds_id=2):
  df = pd.read_csv(f'data/ds_pca_{ds_id}.csv')
  ds = df.to_numpy()[:, 1:]
  if ds_id==1:
    ds -= ds.mean(axis=0, keepdims=True)
    ds /= ds.std(axis=0, keepdims=True)
  ds = ds + np.random.normal(loc=0, scale=eps, size=ds.shape)
